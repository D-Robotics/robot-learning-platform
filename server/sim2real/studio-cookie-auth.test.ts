...
describe('studio web-cloud cookie codec', () => {
  it('round-trips a Studio-compatible cookie payload', () => {
    const encoded = encodeStudioWebCloudCookie(USER, NOW + 60_000, 'robogo-token', {
      secret: SECRET,
    });
  ...
  it('rejects tampered ciphertext (GCM auth tag mismatch)', () => {
    const encoded = String(
      encodeStudioWebCloudCookie(USER, NOW + 60_000, undefined, { secret: SECRET }),
    );
  ...
  it('rejects payloads with an invalid user shape', () => {
    ...
    const encoded = encodeStudioWebCloudCookie(badUser, NOW + 60_000, undefined, {
      secret: SECRET,
    });
    expect(
      decodeStudioWebCloudCookie(String(encoded), { secrets: [SECRET], now: () => NOW }),
    ).toBeNull();
  });
  ...
  it('parses the cookie header the same way Studio does (percent-decoding)', () => {
    ...
    expect(
      parseCookieValue(
        `other=1; ${STUDIO_WEB_CLOUD_SESSION_COOKIE}=${value}`,
        STUDIO_WEB_CLOUD_SESSION_COOKIE,
      ),
    ).toBe(value);
