...
def main():
    req=json.load(open(os.environ['RDK_SIM2REAL_REQUEST_FILE'])); out=os.environ['RDK_SIM2REAL_RESULT_FILE']; profile=req.get('training',{}).get('profile','smoke'); updates={'smoke':20,'low-vram':80,'standard':180,'high-vram':300}.get(profile,20)
    requested=os.environ.get('RDK_STARTER_ENGINE_DEVICE','auto').strip().lower()
    if requested not in ('auto','cpu','cuda'): raise ValueError("RDK_STARTER_ENGINE_DEVICE must be auto, cpu, or cuda")
    cuda_available=torch.cuda.is_available()
    if requested=='cuda' and not cuda_available: print('[originbot] cuda requested but unavailable; refusing GPU claim and using CPU')
    use_cuda=cuda_available and requested != 'cpu'
    device=torch.device('cuda' if use_cuda else 'cpu')
    torch.manual_seed(7); random.seed(7)
    if use_cuda: torch.cuda.manual_seed_all(7)
    env=RDKRobotEnv(seed=7); net=ActorCritic().to(device); opt=torch.optim.Adam(net.parameters(),lr=3e-4); gamma=.99; lam=.95; clip=.2; rewards=[]
    for update in range(updates):
        obs,_=env.reset(seed=7+update); O=[]; A=[]; LP=[]; V=[]; R=[]; D=[]
        for _ in range(128):
            x=torch.tensor([obs],dtype=torch.float32,device=device); mu,val=net(x); dist=torch.distributions.Normal(mu,torch.exp(net.logstd)); a=dist.sample(); lp=dist.log_prob(a).sum(-1); act=a[0].detach().cpu().tolist(); nxt,r,done,_,_=env.step(act)
        ...
        with torch.no_grad(): _,lastv=net(torch.tensor([obs],dtype=torch.float32,device=device)); lastv=lastv[0]
        adv=[]; gae=torch.tensor(0.,device=device); vals=V+[lastv]
        for i in reversed(range(len(R))):
            delta=torch.tensor(R[i],device=device)+gamma*vals[i+1]*(0 if D[i] else 1)-vals[i]; gae=delta+gamma*lam*(0 if D[i] else 1)*gae; adv.insert(0,gae)
    ...
    try: torch.onnx.export(net,torch.zeros(1,8,device=device),os.path.join(job,'originbot-policy.onnx'),input_names=['observation'],output_names=['action','value'],opset_version=17); onnx_exported=True
    except Exception: pass
    json.dump({'status':'completed','checkpoint':{'checkpointId':'originbot-ppo-final','artifactRef':'artifact://originbot/ppo-checkpoint','iteration':updates},'artifact':{'artifactRef':'artifact://originbot/ppo-policy','format':'onnx' if onnx_exported else 'torch','observationSize':8,'actionSize':2},'metrics':{'algorithm':'ppo','rewardStart':rewards[0],'rewardEnd':rewards[-1],'reward':max(rewards),'iterations':updates,'observationSize':8,'actionSize':2,'onnxExported':onnx_exported,'simulator':'originbot-kinematic','device':device.type,'deviceName':torch.cuda.get_device_name(0) if use_cuda else 'cpu'},'cuda':use_cuda,'deployable':False},open(out,'w'))
    print(json.dumps({'status':'completed','algorithm':'ppo','iterations':updates,'rewardStart':rewards[0],'rewardEnd':rewards[-1],'onnxExported':onnx_exported,'device':device.type,'deviceName':torch.cuda.get_device_name(0) if use_cuda else 'cpu'}))
