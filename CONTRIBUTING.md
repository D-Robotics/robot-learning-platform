...
4. 提交前运行 `npm run verify`、`npx tsc --noEmit`、`npm run lint` 与 `npm run format:check`，并检查 `git diff --check`。
...
6. 前端不允许把未转义的值写进 `innerHTML`。`npm run verify:escape-audit` 会扫描 `public/*.js` 的模板插值；确实安全但静态不可判定的值（局部格式化函数、数字）必须用带理由的显式标注豁免：`// escape-audit:allow <理由>`，且注释必须写在模板字面量之外。新增的 HTML 页面不要写内联 `<script>`（CSP 是 `script-src 'self'`），逻辑放到独立 `.js` 文件。
