...
assert.match(
  standalone,
  /createDeviceBoardDetectRouter\(runOnDevice,\s*\{\s*auth:\s*studioSsoAuth\s*\}\)/,
);
...
const adapters = fs.readFileSync(path.join(root, 'server/sim2real/standalone-adapters.ts'), 'utf8');
...
assert.match(
  ...
  'business router must mount the board-station proxy',
);
assert.match(
  stationRoutes,
  /SIM2REAL_BOARD_AGENT_NOT_CONFIGURED/,
  'station proxy must fail closed when no agent is configured',
);
assert.match(
  stationRoutes,
  /SIM2REAL_STATION_COMMAND_REJECTED/,
  'station commands must be whitelisted before dispatch',
);
assert.match(stationRoutes, /requestOwner/, 'station proxy must authenticate and scope by owner');
assert.match(
  stationProxy,
  /boardAgentUrl\(\)/,
  'station proxy must reuse the SSRF-safe agent URL resolver',
);
assert.match(stationProxy, /redirect: 'error'/, 'station proxy must not follow redirects');
assert.match(
  stationProxy,
  /studioBridgeAgentJson/,
  'shared Studio deployments must reuse the Local Bridge adapter',
);
assert.match(
  stationProxy,
...
assert.match(
  tunnelManager,
  /BatchMode=yes/,
  'tunnel ssh must be non-interactive (no passwords in the web layer)',
);
assert.match(
  tunnelManager,
  /ExitOnForwardFailure=yes/,
  'failed forwards must kill the tunnel, not half-open',
);
...
assert.match(
  connectionRoutes,
  /baseUrl: url/,
  "config proxy must target the connection's own tunnel, not the global agent URL",
);
assert.match(
  connectionRoutes,
  /RDK_SIM2REAL_BOARD_AGENT_ENABLE_DRIVE/,
  'only the two documented board switches may be flipped',
);
assert.match(
  stationRoutes,
  /SIM2REAL_STATION_SWITCH_CONFIRM_REQUIRED/,
  'turning motion ON must require an in-band confirm flag',
);
assert.match(
  stationRoutes,
  /stationSwitchEnabled/,
  'drive/policy gates must read the runtime override surface',
);
const stationApp = fs.readFileSync(path.join(root, 'services/sim2real-web/public/app.js'), 'utf8');
assert.match(
  stationApp,
  /device-connections/,
  'the station UI must call the device-connection surface',
);
assert.match(
  stationApp,
  /board-station\/switches/,
  'the station UI must read the platform switch state',
);

console.log(
  '[sim2real-web wiring] PASS — standalone and Studio-bridge surfaces are isolated by explicit adapters',
);
