...
assert.match(
  ...
  'telemetry-core.js must publish globalThis.SimTelemetryCore',
);
assert.doesNotMatch(
  telemetryCore,
  /document\.|window\.location/,
  'telemetry-core.js must stay DOM-free',
);
assert.match(
  app,
...
assert.match(
  app,
  /SimTelemetryCore\.stationTelemetrySnapshot\(status\)/,
  'app.js must delegate stationTelemetrySnapshot',
);
assert.match(
  app,
  /SimTelemetryCore\.stationPowerView\(status\)/,
  'app.js must read the station power surface from one view',
);
assert.match(
  app,
  /SimTelemetryCore\.simulatorStatusLabels\(profile, browserAvailable\)/,
  'app.js must delegate simulator labels',
);
assert.match(
  app,
  /SimTelemetryCore\.stationImuQuaternion\(originbot\)/,
...
assert.match(
  densityCss,
  /\.status-card p \{[^}]*white-space: nowrap/,
  'inline status detail must ellipsize instead of wrapping',
);
...
assert.match(
  stripHtml,
  /id="context-live-model"/,
  'context strip keeps the live model status chip',
);
assert.match(
  stripHtml,
  /id="context-live-device"/,
  'context strip keeps the live device status chip',
);
assert.match(stripHtml, /id="status-storage"/, 'context strip keeps the ledger health chip');

assert.match(
  html,
  /sidebar-project[\s\S]{0,400}id="product-select"/,
  'product selector lives in the sidebar project card',
);
assert.match(html, /id="task-select"/, 'task selector must exist');
assert.match(
  html,
  /guide-panel[\s\S]{0,2000}id="task-select"/,
  'task selector lives on the simulate page',
);
assert.match(
  html,
  /model-panel[\s\S]{0,2000}id="model-select"/,
  'model selector lives on the train page',
);
assert.match(
  html,
  /board-panel[\s\S]{0,2000}id="device-select"/,
  'device selector lives on the deploy page',
);
...
assert.match(
  app,
  /window\.setAgentDrawerOpen === 'function'/,
  'app.js must delegate agent opening to agent-chat.js',
);
assert.match(
  agentChat,
  /window\.setAgentDrawerOpen = /,
  'agent-chat.js must publish setAgentDrawerOpen',
);
assert.match(agentChat, /agent-prompt-chip/, 'agent composer must expose example prompts');
assert.match(
  agentChat,
  /form\.requestSubmit\(\)/,
  'prompt chips must submit the task in one click, not just fill the input',
);
assert.match(
  agentChat,
  /agent-task-card/,
  'agent chat must render a conversation-native task card',
);
assert.match(
  agentChat,
  /setAttribute\('role', 'progressbar'\)/,
  'task card must expose an accessible progress bar',
);
...
assert.match(
  html,
  /id="agent-floating-toggle"/,
  'Agent stays available from a compact floating launcher',
);
...
assert.match(
  app,
  /main-content.*aria-busy/,
  'loading state must be announced to assistive technology',
);
...
assert.doesNotMatch(
  html,
  /section-kicker/,
  'no legacy section kicker labels: keep the workflow surface compact',
);
...
assert.match(
  ...
  'service status dot must follow loading and error state',
);
...
assert.match(
  app,
  /kick-left.*keys: \['Q'\]/,
  'MicroDuck template must publish the Q left-kick binding',
);
assert.match(
  app,
  /kick-right.*keys: \['E'\]/,
  'MicroDuck template must publish the E right-kick binding',
);
assert.doesNotMatch(
  app,
...
assert.match(
  ...
  'external simulator URLs must open safely',
);
assert.match(
  app,
  /configuredBrowserEntry = safeLaunchUrl\(simulator\.browser\?\.entryUrl/,
  'configured simulator entry must drive the iframe',
);
...
assert.match(
  ...
  'synthetic evidence path must stay explicit',
);
assert.match(
  app,
  /function isSyntheticEvidence\(evidence, run\)/,
  'synthetic provenance must survive persisted replay state',
);
assert.match(
  telemetryCore,
  /function hasRealEvaluation\(evidence, run\)/,
  'persisted real evaluation must survive a page refresh',
);
assert.match(
  telemetryCore,
...
assert.match(
  ...
  'telemetry rates must be presentation-safe',
);
assert.match(
  app,
  /已评测 · 摘要已保存/,
...
assert.match(
  app,
  /telemetry:replay:/,
  'saved replay summaries must remain discoverable in audit records',
);
assert.match(
  app,
  /const telemetrySource = acceptedSources\.has\(evidence\.source\)/,
  'fixture uploads must retain provenance',
);
assert.match(app, /Mock 协议演示 · 非真实 RL/, 'mock metrics must be visibly labelled');
assert.match(
  app,
  /已载入遥测证据/,
  'a real local import must not be labelled as synthetic evidence',
);
assert.match(
  app,
  /const performanceMetrics = demoEvidence \|\| mockRun \? \{\} : metrics/,
  'mock performance values must stay out of real metric cards',
);
assert.match(app, /Mock 不可部署/, 'mock runs must stay blocked at the deployment gate');
assert.match(
  app,
  /协议演示完成，继续查看安全闸门/,
  'mock completion must route to the safety gate',
);
assert.match(
  app,
  /pipeline-step.*is-demo|is-demo/,
  'mock workflow state must have a dedicated visual state',
);
assert.match(
  app,
...
assert.match(
  ...
  'demo query must stabilize first-run preferences',
);
assert.match(
  app,
  /headers: \{ 'Idempotency-Key': requestKey \}/,
  'deployment plans must be idempotent',
);
assert.match(
  app,
...
assert.match(
  html,
  /id="telemetry-reward-canvas"/,
  'telemetry panel must expose a reward timeline canvas',
);
assert.match(
  html,
  /id="telemetry-obs-heatmap"/,
  'telemetry panel must expose an observation heatmap',
);
assert.match(
  html,
  /id="telemetry-action-heatmap"/,
  'telemetry panel must expose an action heatmap',
);
...
assert.match(
  ...
  'heatmaps must use a fixed diverging color scale',
);
assert.match(
  app,
  /function renderRunMetricCards\(record\)/,
  'run detail must render readable metric cards',
);
assert.match(
  app,
...
assert.match(
  html,
  /id="station-honesty-note"/,
  'station must surface the mock/reference honesty note',
);
...
assert.match(
  app,
  /function stationRunCommand\(id\)/,
  'station commands must be dispatched through the proxy',
);
assert.match(
  app,
