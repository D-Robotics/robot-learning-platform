...
## 交互反馈与公告（Amershi G12/G15/G18）

- **结果反馈（G15）**：评测结果卡片和运行记录详情提供「准确 / 不准确 + 可选备注」控件，
  Agent 对话不再逐条弹出准确性询问。`POST /api/v1/duck/feedback` 按 owner 归档、容量上限 500 条、备注
  600 字。未知 surface/verdict 降级映射为 `run-record` / `inaccurate` 并带 `degraded`
  标记（旧客户端语义兼容）。反馈仅作运营遥测，不参与发布/晋级闸门。
- **平台公告（G18）**：`GET /api/v1/duck/notices` 匿名返回当前版本公告（版本号取自
  `package.json`，不设第二事实源）与存储降级通知；客户端顶部公告条在版本变化时给出
  「刷新页面」CTA，同一版本可关闭并不再打扰。
- **Agent 会话持久化（G12）**：Agent 对话的最近会话（上限 12 个，每个保存最近 24 条
  消息 + 只读任务卡）存于浏览器 `localStorage`，侧栏可回看历史会话；历史任务卡标注
  「只读回放，不能续跑」，避免误以为可以恢复执行。当前对话可导出为 JSON；DSH 会话在
  同一对话内复用会话标识，后续问题能继续使用上下文。
- 评测页的质量门徽章如实显示服务端 `qualityGate.passed` 结论（Qualified / Unqualified，
  悬停展示判据或失败原因），没有结论时徽章不显示——客户端不发明判据。

## 计划选择、可调建议与反馈汇总（Bakusevych #2/#29/#38、Amershi G13）

- **计划执行方式可选（#2）**：`POST /api/v1/sim2real/agent/plan` 在返回默认 `plan` 的同时
  返回 `variants`——同一目标的确定性重定界：`thorough`（完整，默认）、`fastest`（仅核心
  工具）、`read-only`（过滤全部写工具，保证零副作用）。多于一档时 Agent 对话先弹选择卡
  （标注步数与安全级别，可「先不执行」取消，120 秒无人操作自动取消），选择才提交执行。
  对话类请求保持单档不弹窗。变体由规划器确定性派生，不是随机生成。
- **重训建议可调档（#29）**：运行详情的重训建议除只读阈值外，档位是可编辑下拉框；
  请求体 JSON 预览随档位实时重排，确认框区分「保持建议」与「你已调整，非原始建议」。
  档位改动只在显式提交时进入请求体，平台不会自动发起训练。
- **反馈效果汇总（#38/G17）**：`GET /api/v1/sim2real/feedback/summary` 返回 owner 近
  30 天反馈聚合计数（总量、准确/不准确、按 surface 分组）；总览页「反馈效果汇总」面板
  展开时懒加载，展示每类界面的准确占比条。聚合只含计数，不含备注、run ID 或任何身份
  标识，保持遥测-only 契约。
- **训练档位显式偏好（G13）**：上次选过的训练档位写入工作区偏好（与模型/设备同一
  localStorage 键），下次进入训练页预选并标注「已按你上次的选择预选」——预选永远可见、
  一键可改，不静默生效。
