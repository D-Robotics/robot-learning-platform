...
- **工程护栏（9+）**：ESLint（error 级为真实缺陷规则，零违规）与 Prettier/EditorConfig 进 CI；Node 20/22/24 矩阵；Node 22 CI 使用 Vitest 4.1.11 运行 `test:coverage`，执行 lines 71、statements 68、functions 74、branches 61 的下限（V8 remapper 升级后按新的基线校准）；同一门禁执行全依赖 moderate 审计、生产依赖 high 审计和高置信公开面秘密扫描；Dependabot 覆盖 npm / GitHub Actions / pip。
