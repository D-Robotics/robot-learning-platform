...
> **分级完成语义**（借鉴 Studio agent-friendly spec）：本清单只区分两级声明——
> **「实现已验收」**：对应 focused gate + `verify:ui` + `npm test` 在最终源 SHA 上全绿，
> 且结论绑定命令输出；**「可公开发布」**：在验收之上，本页全部阻塞项由证据（而非勾选）
> 支撑。任何一项未打勾，就不应把仓库标记为"可公开发布"或"整体 9+"。
