...
隧道 + station 代理路由下的策略 load/stop 全链路**。两轮都未运动；策略驱动
低速运动已于同日 20:04 补验 PASS（见下），任务级到达点验收仍待板子回电
（runbook：`docs/goalnav-task-acceptance-runbook.md`）。
...
| 板端遥测/推理链路修复（4 处） | ✅ |
| 双发布器互踩根因确诊 + 修复 | ✅ 修复已部署 |
| **策略驱动真机运动窗口** | ✅ 同日 20:04 补验 PASS——0.096m 弧线、51 次真实推理、急停生效（本表为 19:40 时点快照，结果见下「策略运动窗口（修复双发布器后 · ✅ PASS）」节） |

真机低速运动**已经发生并被三重证据确认**（驱动路径）；策略路径的运动验证
曾受双发布器缺陷阻断，修复后已于同日 20:04 补验通过（见下节）。仍待板子
回电的是**任务级到达点验收**（42D goal 装配缺口 2026-09-17 已修复，部署与
验收流程见 `docs/goalnav-task-acceptance-runbook.md`）。
