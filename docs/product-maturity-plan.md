...
| 机器人仿真与训练 | NVIDIA Isaac Lab 侧重高保真 GPU 仿真、多 GPU 训练和丰富任务/机型生态 | 侧重契约驱动、可复核 Task-Pack、CPU starter 和可替换训练适配器；已含 MJX 引擎（真 MuJoCo 接触动力学 + 物理级域随机化 + 纯 JAX PPO，CPU 可验证、GPU 放大吞吐）、视觉观测训练（visual-ppo）、dm_control 生态适配与 mujoco-web 部署方模型注册表 | GPU 集群编排（mjlab 生产算力属部署方）、更多任务和真实 sim-to-real 复现 |
