...
```text
选择 Duck 套件 → 仿真与录制 → 强化学习训练 → Sim2Real 评测 → 部署与反馈 → 看效果与回滚
```
...
平台固定为“一个总览 + 四个流程页 + 两个数据与工具页”，Agent 作为可随时打开的任务抽屉；左侧导航始终可见：

1. **工作台总览**：当前项目、下一步、集成健康度、最近活动和 Agent 建议。
2. **仿真与录制**：浏览器仿真、动作说明、开始/停止/下载/回放。
3. **强化学习训练**：选择模型、套件与契约、Local/Mock/RoboGo、训练档位和制品。
4. **Sim2Real 评测**：仿真/真机指标、成功率、跌倒率、延迟和遥测证据。
5. **部署与反馈**：设备选择、板型探测、只读预检、Canary/Live 闸门。
6. **调试与记录**：运行历史、checkpoint、评测报告、部署和审计记录。
7. **设备上位机**：板卡状态、遥测、相机流、诊断命令和受限安全控制。

模型契约不是独立导航页，而是训练页中的“套件与契约”折叠面板；Agent 对话也不占用工作流编号。

项目和设备选择放在全局上下文条，避免用户在不同页面重复选择，也避免把“当前模型”藏在某个卡片里。

页面采用“两层信息架构”，把引导和能力拆开：

- **任务流层**：顶部固定显示“仿真与录制 → 强化学习训练 → Sim2Real 评测 → 部署与反馈”，适合第一次使用或按顺序交付一个动作。
- **平台模块层**：总览页提供上述流程入口、调试与记录、设备上位机和 Agent 对话；熟悉用户可以直接进入任一模块，失败后也只重跑当前模块。
