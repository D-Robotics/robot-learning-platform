...
RDK Robot Learning Platform（RDK Duck Lab）是一个独立 Web 控制面。它把“一个动作任务”拆成可理解、可回放、可审计的步骤：选择产品线与项目 → 登记契约 → 仿真与录制 → 强化学习训练 → Sim2Real 评测 → 部署与反馈 → 看证据与回滚。
...
当前平台是面向 RDK 自有 Duck 的策略证据链与安全交付底座。仿真资源、机器人模型、动作空间、传感器和板端运行时均通过 profile 与 manifest 接入，平台不会把某个第三方机器人写死在业务流程中。OriginBot 作为独立的移动底盘产品线接入：它拥有自己的差速 adapter、8→2 目标导航契约、PPO 训练器和 X5 运行时；OriginBot 的真实结果仍不能直接代表 RDK Duck 的关节运动学、控制接口或评测结论。
...
*图 2：总览、四个流程页、两个数据与工具页和全局上下文共同构成工作台。*

顶部任务流始终显示“仿真与录制 → 强化学习训练 → Sim2Real 评测 → 部署与反馈”；左侧导航按“核心 / 流程 / 数据与工具”分组，Agent 以抽屉形式随时打开。上下文条统一保存项目、产品线、动作任务、当前模型、目标设备和账号状态，页面之间通过 `projectId / modelId / deviceId` 传递状态。

| 编号 | 模块 | 主要能力 | 典型产出 |
|---|---|---|---|
| 00 | 总览 | 集成健康度、当前阶段、下一步建议 | 工作流状态 |
| 01 | 仿真与录制 | RDK Duck 仿真、键盘/按钮控制、轨迹录制和回放 | JSON/JSONL 轨迹 |
| 02 | 强化学习训练 | 选择后端、训练档位、续训、查看指标 | Run、checkpoint、ONNX |
| 03 | Sim2Real 评测 | 导入遥测、仿真/真机对比、回放摘要 | MAE/RMSE、成功率、跌倒率 |
| 04 | 部署与反馈 | 设备选择、板型探测、预检和发布闸门 | Preflight、Canary 计划 |
| 05 | 调试与记录 | Runs、制品、发布、遥测的筛选和审计 | 可追溯记录 |
| 06 | 设备上位机 | CPU/内存/网络/电源、相机流、白名单只读命令 | 状态心跳、MJPEG |
| — | 套件与契约 | 训练页折叠面板：登记 manifest、配件和契约检查 | 版本化模型契约 |
...
每个模型版本必须声明产品标识、观测布局、动作布局、控制频率、运行时、目标板型、制品引用和校验和。MicroDuck 的官方参考契约是 **61D observation → 14D action，50 Hz**；RDK Duck 使用自身 manifest 契约（仓库内 starter 模板为 42D/12D 参考环境，不代表真实硬件维度）。策略制品使用不透明 `artifact://` 引用；普通 ONNX 只代表仿真/本地路径，上板必须提供匹配 X5 的编译制品和 runtime 元数据。
...
**怎么使用**：登录后先在侧栏项目卡选择项目和产品线，在各流程页分别选择动作任务、模型和目标设备；查看四阶段状态卡，点击“下一步”进入当前最早可执行的模块；需要现场讲解时使用 `?demo=1` 固定 MicroDuck 演示上下文。
...
**怎么使用**：进入“强化学习训练”→展开“套件与契约 · 登记模型”→选择产品线→载入模板/导入 manifest→执行“校验清单”→确认观测、动作、频率和运行时→登记版本。导入制品时同时记录 artifact 引用和校验和。
...
**怎么使用**：进入“强化学习训练”→选择已登记模型→选择 Local、Mock 或 RoboGo→选择 smoke/low-vram/standard/high-vram→确认环境数和最大迭代→发起训练→在 Run 卡片查看排队、运行和完成状态→打开详情下载 checkpoint 或 ONNX。
...
**怎么使用**：进入“Sim2Real 评测”→选择 Run→导入浏览器或 X5 JSON/JSONL 遥测→本地检查采样率和事件→查看奖励曲线、成功率、跌倒率、延迟和资源占用→点击“上传到当前 Run 并评测”→生成对比摘要。
...
**怎么使用**：进入“部署与反馈”→选择目标设备→点击“探测板型”→确认设备 passport→生成预检计划→执行只读预检→查看契约、runtime、磁盘和设备能力→预检通过后创建 Canary 计划→人工批准后进入 Live。
...
**怎么使用**：进入“调试与记录”→按 Runs、制品、发布或遥测筛选→按模型名、任务名或 Run ID 搜索→打开详情查看指标卡、原始 JSON、来源和时间线→将关键版本链接到评测报告。
...
**当前边界**：浏览器 OriginBot 页面使用隔离的 MuJoCo 物理服务，可验证差速底盘、传感器和域随机化的流程；它仍不等价于真实 X5 的接触、摩擦、标定误差和传感器噪声。实时看板里的内置参考轨迹仍是独立的运动学样例，不能当作真机误差评测。要得到硬件结论，仍需真实 OriginBot 遥测、板端 attestation 和现场回归。
