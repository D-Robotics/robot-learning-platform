...
const required = [
  [
    ...
    ['RDK_SIM2REAL_BOARD_AGENT_TOKEN', '/v1/station/policy/load', 'drive/stop'],
  ],
  ...
  [
    ...
    ['EnvironmentFile=', 'Restart=always', 'ReadWritePaths='],
  ],
  [
...
const py = spawnSync(
  ...
  [
    '-m',
    'py_compile',
    ...required.filter(([f]) => f.endsWith('.py')).map(([f]) => path.join(root, f)),
  ],
  { encoding: 'utf8' },
);
if (py.status !== 0) throw new Error(py.stderr || 'python compile failed');
console.log(
  '[real-loop] PASS — telemetry shape, policy watchdog state, dimension gate, durable uploader, and board agent wiring verified',
);
