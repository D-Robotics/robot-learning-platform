...
const required = async (path) => {
  ...
  try {
    ...
    // Body is optional diagnostics for the error message below.
  }
  if (!response.ok)
    throw new Error(`${path}: HTTP ${response.status} ${body?.code || body?.error || ''}`.trim());
  return body;
};
try {
  ...
  if (!status?.originbot || !status?.originbot.imu)
    throw new Error('real OriginBot IMU telemetry is missing');
  if (status?.power == null && status?.originbot?.batteryVoltage == null)
    throw new Error('real battery telemetry is missing');
    ...
    JSON.stringify(
      {
        ...
        base,
        board: status.board,
        battery: status.power ?? status.originbot.batteryVoltage,
