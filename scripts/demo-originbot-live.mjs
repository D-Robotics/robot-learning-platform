...
try {
  ...
  kv(
    '设备',
    healthBody?.device
      ? `${healthBody.device.name} (${healthBody.device.id}) · ${healthBody.device.status}`
      : '—',
  );
  kv(
    '板型',
    healthBody?.device
      ? `${healthBody.device.boardPlatform} · ${healthBody.device.boardModel}`
      : '—',
  );
  ...
  if (agentMock) {
    console.log(
      `  ${yellow('⚠ 当前是本地参考 agent（mock=true，模拟遥测）——适合彩排，不是真机数据')}`,
    );
    if (flag('require-real')) {
      console.log(realBoardHint);
      process.exit(2);
    }
  } else {
  ...
  if (!status) {
    console.log(
      `  ${yellow('⚠ 遥测不可达')}: HTTP ${snap.status} ${snap.body?.code || snap.body?.error || ''}`.trim(),
    );
  } else {
    summary.telemetry = {
    ...
    kv(
      '板卡',
      `${status.board?.model ?? '—'} ${status.board?.mock === false ? green('(mock=false)') : yellow('(mock=true)')}`,
    );
    ...
    kv(
      '网络',
      `rx ${fmt(status.network?.rxKbPerSec, 0)} KB/s · tx ${fmt(status.network?.txKbPerSec, 0)} KB/s`,
    );
    const battery = volts(status);
    kv(
      '电源',
      battery != null
        ? `${fmt(battery)} V${status.power?.current != null ? ` · ${fmt(status.power?.current)} A` : ''}`
        : '—',
    );
    kv(
      'IMU 航向',
      status.originbot?.imu
        ? `yaw ${fmt(yawFromQuaternion(status.originbot.imu), 1)}°（四元数实时解算）`
        : '—',
    );
    kv(
      '里程计',
      status.originbot?.odom
    ...
    kv(
      'TROS 话题',
      status.topics
        ? `${status.topics.length} 个（${status.topics
            .slice(0, 4)
            .map((t) => t.name)
            .join(' ')}${status.topics.length > 4 ? ' …' : ''}）`
        : '—',
    );
  ...
  if (policy.body?.ok) {
    ...
    kv(
      '策略运行时',
      `${p.enabled ? 'enabled' : 'disabled'} · ${p.runtimeRunning ? 'running' : 'stopped'} · state=${p.state ?? 'null'}`,
    );
    ...
    kv(
      '策略状态',
      `${yellow('不可达')} — HTTP ${policy.status} ${policy.body?.code || policy.body?.error || ''}`.trim(),
    );
  }
  ...
  if (drive.body?.ok) {
    ...
    if (drive.body.actuatorPolicy)
      kv('钳制', `maxLinear ${fmt(drive.body.actuatorPolicy.maxLinear)} m/s · 时长盒 0.2–2.0 s`);
    if (!drive.body.platformEnabled || !d.enabled) {
      console.log(
        dim(
          '  驱动双开关未全开:POST /drive 会被 409 拒绝(不触达板端)。开启方法见 docs/actuator-drive.md。',
        ),
      );
    }
  } else {
    kv(
      '驱动状态',
      `${yellow('不可达')} — HTTP ${drive.status} ${drive.body?.code || drive.body?.error || ''}`.trim(),
    );
  ...
  if (healthBody?.cameraSupported)
    kv('相机 MJPEG', `${base}/api/sim2real/board-station/camera.mjpeg`);
  ...
  if (opt('watch')) {
    ...
    for (let i = 0; i < seconds; i++) {
      try {
        ...
        console.log(
          `  ${new Date().toLocaleTimeString('zh-CN', { hour12: false })}  bat ${fmt(volts(s))}V  yaw ${fmt(yawFromQuaternion(imu), 1)}°  odom (${fmt(odom.positionX)}, ${fmt(odom.positionY)}) v=${fmt(odom.linearX)} w=${fmt(odom.angularZ)}  cpu ${fmt(s.cpu?.percent, 1)}% ${fmt(s.cpu?.temperatureC, 1)}°C`,
        );
      } catch {
        console.log(
          `  ${new Date().toLocaleTimeString('zh-CN', { hour12: false })}  ${yellow('… 不可达')}`,
        );
      }
  ...
  if (flag('canary')) {
    ...
    if (!flag('present')) {
      console.log(
        `${red('✗ 拒绝执行')}: 金丝雀要求人在场。确认操作者在机器人旁、场地清空、急停可用后，加 --present 重试。`,
      );
      ...
      if (!gates.body?.ok) {
        console.log(
          `${red('✗ 驱动状态不可达')} — HTTP ${gates.status} ${gates.body?.code || gates.body?.error || ''}。`.trim(),
        );
        console.log(dim('  无法确认双开关状态，拒绝执行且未向板端发送任何运动命令。'));
        summary.exitCode = 5;
      } else if (gates.body.platformEnabled !== true || gates.body.drive?.enabled !== true) {
        console.log(
          `${red('✗ 闸门未全开')}（平台 ${gates.body.platformEnabled ? 'on' : 'off'} / 板端 ${gates.body.drive?.enabled ? 'on' : 'off'}）——按设计拒绝，未向板端发送任何运动命令。`,
        );
        console.log(
          dim(
        ...
        console.log(
          dim('        平台侧 .env 加 RDK_SIM2REAL_STATION_DRIVE_ENABLED=1 并重启工作台。'),
        );
        summary.exitCode = 5;
      } else {
        const post = (body) =>
          api(
            base,
            '/api/sim2real/board-station/drive',
            {
              method: 'POST',
        ...
        try {
          ...
          for (const [label, body] of steps) {
            ...
            if (res.status >= 200 && res.status < 300 && res.body?.ok !== false) {
              ...
              console.log(
                `  ${red('✗')} ${label}: HTTP ${res.status} ${res.body?.code || res.body?.error || ''} ${res.body?.reason ? `· ${res.body.reason}` : ''}`.trim(),
              );
              summary.exitCode = 6;
              break;
            }
            ...
            console.log(
              dim(
                `    窗口后: active=${after.body?.drive?.active ?? '—'} lastStopReason=${after.body?.drive?.lastStopReason ?? '—'}`,
              ),
            );
          }
        } finally {
          ...
          if (interrupted) {
            console.log(yellow('  （被 Ctrl+C 中断，已执行急停）'));
            summary.exitCode = 4;
          }
        }
      }
