 ...
 *
 * Usage:
 *   node scripts/onboard-gpu-server.mjs --host 10.0.0.5 --user robot --json
 */
...
export function parseArgs(argv, env = process.env) {
  ...
  if (!flags.help && (!validHost(flags.host) || !validUser(flags.user) || !validPort(flags.port) || !validRemoteDir(flags.dir))) {
    throw new Error('invalid --host/--user/--port/--dir; values are validated and shell metacharacters are rejected');
  }
...
function probeCommand(flags, command, timeout = 12_000) {
  ...
  const stdout = String(result.stdout ?? '').slice(0, MAX_OUTPUT).trim();
  const stderr = String(result.stderr ?? '').slice(0, MAX_OUTPUT).trim();
  return {
    ok: result.status === 0,
...
export function runOnboarding(flags, runner = probeCommand) {
  const run = (name, command, parse) => {
    ...
    return { name, ok: probe.ok, ...((parse && parse(probe)) || { detail: probe.stdout || probe.stderr }), exitCode: probe.exitCode, timedOut: probe.timedOut };
  };
  const remoteDir = shellQuote(flags.dir);
  const checks = [
    run('ssh', 'printf %s ready', (p) => ({ detail: p.stdout, error: p.ok ? undefined : p.stderr || 'ssh probe failed' })),
    run('gpu', 'command -v nvidia-smi >/dev/null && nvidia-smi --query-gpu=name,memory.total,driver_version --format=csv,noheader', (p) => ({ detail: p.stdout, error: p.ok ? undefined : p.stderr || 'nvidia-smi unavailable' })),
...
function printHuman(result) {
  console.log(`GPU server onboarding: ${result.ok ? 'READY' : 'BLOCKED'} (${result.user}@${result.host}:${result.port})`);
  for (const check of result.checks) console.log(`  ${check.ok ? '✓' : '✗'} ${check.name}${check.detail ? ` — ${check.detail}` : ''}`);
...
async function main() {
  try {
    ...
    if (flags.help) {
      console.log('usage: node scripts/onboard-gpu-server.mjs --host <host> --user <user> [--port 22] [--dir ~/rdk-sim2real] [--json]');
      return;
    }
